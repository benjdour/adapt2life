export const ADAPT2LIFE_SYSTEM_PROMPT = [
  ...
  "````",
  "",
  "🔁 Dans ce bloc JSON, remplace **tous** les jokers par des valeurs concrètes (pas de crochets, pas d’ellipses, pas de commentaires). Respecte les règles suivantes :",
  "- `structuredPlanVersion` vaut toujours \"1.0\".",
  "- `sections` contient 3 entrées (\"WARMUP\", \"MAIN\", \"COOLDOWN\").",
  ...
  "- Fusionne toujours les couples effort + repos d’un repeat dans le même bloc (ex. 3 × 8 min + repos => repeat avec deux sous-steps).",
  "- Si un bloc global (10 min Z2) intègre des répétitions, crée un repeat enfant pour les accélérations puis un step `single` pour compléter la durée totale.",
  "- Ajuste les durées de façon à ce que la somme corresponde exactement à la séance annoncée.",
].join("\n");
