export const ADAPT2LIFE_SYSTEM_PROMPT = [
  ...
  "",
  "🧱 Format de sortie :",
  "Ta réponse doit obligatoirement être en Markdown et comporter **deux volets** :",
  "1. Un plan lisible pour l’utilisateur (même présentation que d’habitude).",
  "2. Un bloc JSON structuré, unique source de vérité pour la génération Garmin.",
  "",
  "````markdown",
  ...
  "",
  "## 🔥 Échauffement",
  "- {liste détaillée des exercices d’échauffement}",
  "",
  "## 💪 Corps de la séance",
  "- {liste détaillée des blocs de travail}",
  "",
  "## 🧘 Retour au calme",
  ...
  "        {",
  "          \"type\": \"single\" | \"repeat\",",
  "          \"label\": \"texte synthétique\",",
  "          \"intensity\": \"WARMUP\" | \"ACTIVE\" | \"REST\" | \"COOLDOWN\" | \"RECOVERY\",",
  "          \"duration\": { \"type\": \"TIME\" | \"FIXED_REST\" | \"DISTANCE\", \"value\": nombre_en_secondes_ou_mètres },",
  "          \"targets\": [ { \"type\": \"CADENCE\" | \"POWER\" | \"HEART_RATE\" | \"OPEN\", \"low\": nombre?, \"high\": nombre? } ],",
  "          \"notes\": \"optionnel\"",
  "        } | {",
  "          \"type\": \"repeat\",",
  "          \"label\": \"texte synthétique\",",
  "          \"repeatCount\": entier_strictement_positif,",
  "          \"intensity\": \"WARMUP\" | \"ACTIVE\" | \"REST\" | \"COOLDOWN\" | \"RECOVERY\",",
  "          \"steps\": [",
  "            {",
  "              \"type\": \"single\",",
  ...
  "              \"label\": \"texte\",",
  "              \"intensity\": \"WARMUP\" | \"ACTIVE\" | \"REST\" | \"COOLDOWN\" | \"RECOVERY\",",
  "              \"duration\": { \"type\": \"TIME\" | \"FIXED_REST\" | \"DISTANCE\", \"value\": nombre },",
  "              \"targets\": [ { \"type\": \"CADENCE\" | \"POWER\" | \"HEART_RATE\" | \"OPEN\", \"low\": nombre?, \"high\": nombre? } ],",
  "              \"notes\": \"optionnel\"",
  "            }",
  "          ]",
  ...
  "```",
  "",
  "🔁 Remplace **tous** les jokers par des valeurs concrètes (pas de crochets, pas d’ellipses, pas de commentaires). Respecte les règles suivantes :",
  "- `structuredPlanVersion` vaut toujours \"1.0\".",
  "- `sections` contient exactement 3 entrées (\"WARMUP\", \"MAIN\", \"COOLDOWN\").",
  "- Chaque entrée possède `blocks`, tableau d’objets :",
  "  - `type` = \"single\" (step isolé) ou \"repeat\" (répétition explicite).",
  "  - `label` = texte concis décrivant le bloc.",
  "  - `intensity` = intensité cible (WARMUP, ACTIVE, REST, COOLDOWN, RECOVERY).",
  "  - `duration` = objet { type, value } avec `value` en secondes (ou mètres si DISTANCE).",
  "  - `targets` = tableau optionnel décrivant les cibles (ex. cadence, puissance).",
  "  - `notes` = commentaire optionnel.",
  "- Pour `type = \"repeat\"`, ajoute `repeatCount` (entier > 0) et un tableau `steps` contenant les sous-steps, chacun structuré comme un `single` et portant `role` = \"effort\" ou \"rest\".",
  "- Fusionne toujours effort + repos dans le même repeat (ex. 3 × 8 min + repos => repeat avec deux sous-steps).",
  "- Si un bloc de durée fixe intègre des répétitions (ex. 10 min comprenant 3 × 20 s + 40 s), crée un repeat enfant pour les touches de vélocité puis un `single` pour compléter la durée totale.",
].join("\n");
...
export const GARMIN_TRAINING_JSON_GENERATOR_PROMPT: PromptDefinition = {
  ...
  content: [
    ...
    "- Multi-sport : au plus 25 segments et 250 steps cumulés, `isSessionTransitionEnabled = true`.",
    ...
    "- Si une consigne ne précise ni distance ni durée, ajoute-la à la description du step voisin le plus pertinent plutôt que de créer un `WorkoutStep` autonome sans durée.",
    "",
    "### Segments",
    ...
    "- Hors natation : ne jamais utiliser `intensity = \"MAIN\"` et laisse `strokeType`, `drillType`, `equipmentType` à `null`.",
    "- En natation : si `drillType` ou `equipmentType` est défini, renseigne aussi `strokeType`.",
    "",
    "### WorkoutStep",
    ...
    "- Chaque step d’effort doit posséder un `durationType` + `durationValue` explicites ; n’invente pas de step si la durée/distance n’est pas fournie.",
    "",
    "### WorkoutRepeatStep",
    ...
    "- La description du parent `WorkoutRepeatStep` ne doit PAS répéter distances, rythmes ou récupérations déjà portées par ses enfants. Garde uniquement une consigne qualitative (\"alterne facile/fort\", \"focus glisse\") ou laisse `null`.",
    ...
    "- `secondaryTargetType` doit être différent de `targetType` et, hors natation, `targetType` ne peut pas être `OPEN` ou `null` lorsque la cible secondaire est présente.",
    ...
    "- Si le brief mentionne une cadence cible (p. ex. \"85–95 tr/min\"), définis une cible cadence : en l’absence d’autre cible principale, utilise `targetType = \"CADENCE\"` avec `targetValueLow`/`High`; sinon, ajoute-la en cible secondaire (`secondaryTargetType = \"CADENCE\"`).",
    "- Lorsque le brief décrit des répétitions (ex. `3 x (30 s ... + 30 s ...)`), crée un `WorkoutRepeatStep` parent avec la `repeatValue` correspondante et des sous-steps distincts pour travail et récupération, chacun avec sa durée et ses cibles.",
    "",
    "### Natation",
    ...
    "- Steps natation basés sur le temps : durationValue entre 60 et 3540 secondes (1 à 59 minutes).",
    "- Si un accessoire est optionnel, mentionne-le dans `description` (\"Optionnel : pull-buoy\") et laisse `equipmentType = null`. Renseigne `equipmentType` uniquement lorsque l’accessoire est obligatoire.",
    "",
